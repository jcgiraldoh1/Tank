...
void Joystick::DoJoystick(void)
{
	if (foundJoystick)
	{
		...
		if (PollJoystick())
		{
			...
			if (joystickState.axisCount > 0) leftJoystickX = joystickState.axisStates[0];
			if (joystickState.axisCount > 1) leftJoystickY = joystickState.axisStates[1];
			if (joystickState.axisCount > 2) rightJoystickX = -joystickState.axisStates[2];
			if (joystickState.axisCount > 3) rightJoystickY = joystickState.axisStates[3];
			if (joystickState.axisCount > 4) leftBumper = joystickState.axisStates[4];
			if (joystickState.axisCount > 5) rightBumper = joystickState.axisStates[5];
